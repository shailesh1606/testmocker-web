...
async def run_openai_extraction(qp_text: str, ak_text: Optional[str], num_questions: int) -> dict:
    ...
    if ak_text:
        system_prompt = (
            ...
            "Use the Answer Key text to map question numbers to their correct answers. "
            ...
            "You must return a strict JSON response containing a 'questions' array. Each item must contain:\n"
            "- 'question_number' (integer)\n"
            "- 'question_type' (string, either 'mcq', 'numeric', or 'text')\n"
            "- 'correct_answer' (string or null). For MCQ, return only the option letter ('A', 'B', 'C', 'D'). For numeric, return the numeric answer (e.g., '4.5' or '10'). For text, return a short phrase.\n"
...
async def extract_answers_from_pdf(qp_path: str, ak_path: Optional[str], num_questions: int) -> List[dict]:
